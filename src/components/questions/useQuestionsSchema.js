...
function useQuestionsSchema({
  questions,
  grade = "",
  tags,
  type = "mcq",
  metaData = { isAdd: true, isDelete: true },
}) {
  ...
  const inputs = [
    {
      ...
      addLabeltAf: metaData.isAdd && "إضافه سؤال صواب وخطأ",
      tAfSchema,
      ...
      type: "chunk",
      array: [
        //inputs control to values of schema
        {
          name: "grade",
          label: lang.GRADE,
          type: "select",
          options: makeArrWithValueAndLabel(grades, {
            value: "index",
            label: "name",
          }),
              ...
              .test({
                ...
                test: (file) => {
                  if (file && file.size) {
                    const isValid =
                      file?.size <=
                      (import.meta.env.VITE_MAX_IMAGE_SIZE_ADMIN || 15) *
                        1024 *
                        1024; // 15MB
                    return isValid;
                  } else {
